...
<body>
  <h1>Editing the selected user</h1>
    <form action= "{{route('modifyusers',[$pizza->id])}}" method="POST" >
      @csrf <!-- {{ csrf_field() }} -->
  ...
  <label for="admin">Hadmin</label>
  <input type="checkbox" name="admin" value="{{($pizza->admin== 1 ? ' checked' : '')}}"><br>

  <label for="updated_at">updated_at:</label><br>
  <input type="datetime" id="updated_at" name="updated_at" value="{{$pizza->updated_at}}"><br>
  <label for="created_at">created_at:</label><br>
  <input type="datetime" id="created_at" name="created_at" value="{{$pizza->created_at}}"><br> 
  <div class="gombok_2">
